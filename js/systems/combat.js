class CombatSystem {
    ...
    buildEnemyMesh(enemy) {
        // Body - use fallback if CapsuleGeometry not available
        let bodyGeometry;
        try {
            if (THREE.CapsuleGeometry) {
                console.log('Creating enemy body with CapsuleGeometry');
                bodyGeometry = new THREE.CapsuleGeometry(0.4, 1.2, 4, 8);
            } else {
                console.log('CapsuleGeometry not available, using fallback for enemy');
                // Fallback to cylinder with spheres
                const cylinder = new THREE.CylinderGeometry(0.4, 0.4, 1.2, 8, 1, true);
                const sphere = new THREE.SphereGeometry(0.4, 4, 8);
            ...
            console.error('Error creating enemy body geometry:', error);
            // Use simple cylinder as last resort
            bodyGeometry = new THREE.CylinderGeometry(0.4, 0.4, 1.2, 8, 1, true);
        }
...
// Make CombatSystem class available globally
window.CombatSystem = CombatSystem;
console.log('CombatSystem class loaded and available globally');
